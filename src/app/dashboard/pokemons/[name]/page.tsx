...
export async function generateStaticParams() {
  const data: PokemonsResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=50`)
    .then(res => res.json());
  const pokemonsName = data.results.map(pokemon => ({ name: pokemon.name }));
  return pokemonsName.map(({ name }) => ({ name: name }));;
}
...
const getPokemon = async (name: string): Promise<Pokemon> => {
  try {
    const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, {
      // cache:'force-cache',
      next: {
        revalidate: 60 * 60 * 30 // For 30 days
      }
    }).then(resp => resp.json());
    return pokemon;
  } catch (error) {
